...
class CRCServer(object):
    ...
    def setup_server_socket(self):
        ...
        data['id'] = self.id
        data['write_buffer'] = b''
        self.sel.register(tcpServer, selectors.EVENT_READ, data)
    ...
    def connect_to_server(self):
        ...
        data['id'] = self.id
        data['write_buffer'] = b''
        self.sel.register(tcpClient, selectors.EVENT_READ | selectors.EVENT_WRITE, data)
        server_reg_message = ServerRegistrationMessage.bytes(self.id, 0, self.server_name, self.server_info)
        tcpClient.send(server_reg_message)
    ...
    def check_IO_devices_for_messages(self):
        ...
        while not self.request_terminate:
            # TODO: Implement the above functionality
            ready_IO = self.sel.select(timeout=1)
            for socks, event_mask in ready_IO:
                if event_mask & selectors.EVENT_READ:
                    self.accept_new_connection(socks)
                else:
                    self.handle_io_device_events(socks, event_mask)
    ...
    def cleanup(self):
        # TODO: Implement the above functionality
        connected_sockets = list(self.sel._fd_to_keys.values())
        for shutdown in connected_sockets:
            self.sel.unregister(shutdown)
            shutdown.close()
    ...
    def accept_new_connection(self, select_key):
        # TODO: Implement the above functionality
        conn, addr = select_key.accept()
        self.sel.register(conn, selectors.EVENT_READ|selectors.EVENT_WRITE,ConnectionData())
    ...
    def handle_io_device_events(self, io_device, event_mask):
        # TODO: Implement the above functionality
        if (event_mask & selectors.EVENT_READ) & (event_mask & selectors.EVENT_WRITE):
            try:
                data = io_device.recv()
            ...
            if len(io_device['write_buffer']) > 0:
                io_device.send(io_device['write_buffer'])
                io_device['write_buffer'] = b''

        elif (event_mask & selectors.EVENT_READ) & (event_mask != selectors.EVENT_WRITE):
            try:
                data = io_device.recv()
                self.handle_messages(data)
            except:
        ...
        elif (event_mask & selectors.EVENT_WRITE) & (event_mask != selectors.EVENT_READ):
            if len(io_device['write_buffer']) > 0:
                io_device.send(io_device['write_buffer'])
                io_device['write_buffer'] = b''
    ...
    def send_message_to_host(self, destination_id, message):
        if destination_id in self.hosts_db:
            ...
            # TODO: Implement the above functionality
            connected_sockets = list(self.sel._fd_to_keys.values())
            for socket_name in connected_sockets:
                if socket_name.id == destination_id:
                    socket_name.send(message)
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        # TODO: Implement the above functionality
        last_hop_ID = message[1].decode()
        if last_hop_ID != ignore_host_id:
            for socket_name in self.adjacent_server_ids:
                if socket_name.id == ignore_host_id:
                    socket_name.send(message)
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        ...
        last_hop_ID = message[1].decode()
        if last_hop_ID != ignore_host_id:
            for socket_name in self.adjacent_user_ids:
                if socket_name.id == ignore_host_id:
                    socket_name.send(message)
    ...
    def handle_server_registration_message(self, io_device, message):
        # TODO: Implement the above functionality
        if io_device.id in self.hosts_db:
            message2send = "A machine has already registered with ID " + io_device.id
            status_msg = StatusUpdateMessage.bytes(self.id, 0, 0x02, message2send)
            self.send(status_msg)
        else:
            newServer = ServerData(message[0], message[2], message[3])
            newServer.first_link_id = 0
            self.hosts_db.append(newServer)
            self.sel.modify(io_device.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, newServer)

            if message[1] == 0:
                for i in self.hosts_db:
                    if i.isinstance() == ClientData:
                        clientmsg = ClientRegistrationMessage.bytes(i.id, i.first_link_id, i.client_name, i.client_info)
                        self.send(clientmsg)
                    elif i.isinstance() == ServerData:
                        servermsg = ServerRegistrationMessage.bytes(i.id, i.first_link_id, i.server_name, i.server_info)
                        self.send(servermsg)
    ...
    def handle_client_registration_message(self, select_key, message):
        # TODO: Implement the above functionality
        if select_key.id in self.hosts_db:
            message2send = "A machine has already registered with ID " + select_key.id
            status_msg = StatusUpdateMessage.bytes(self.id, 0, 0x02, message2send)
            self.send(status_msg)
            return
        else:
            newClient = ClientData(message[0], message[2], message[3])
            newClient.first_link_id = 0
            self.hosts_db.append(newClient.id)
            self.sel.modify(select_key.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, newClient)
            ...
            if message[1] == 0:
                for i in self.hosts_db:
                    if i.isinstance() == ClientData:
                        clientmsg = ClientRegistrationMessage.bytes(i.id, i.first_link_id, i.client_name, i.client_info)
                        self.send(clientmsg)
                    elif i.isinstance() == ClientData:
                        client = ClientRegistrationMessage.bytes(i.id, i.first_link_id, i.client_name, i.client_info)
                        self.send(clientmsg)
    ...
    def handle_status_message(self, select_key, message):
        # TODO: Implement the above functionality
        if (select_key.id == message[1].decode()) | (message[1].decode == 0):
            self.status.updates.log.append(message)
        else:
            ids = list(self.hosts_db.keys)
            for id in ids:
                if id == message[1].decode():
                    self.hosts_db[id].send(message)
    ...
    def handle_client_chat_message(self, select_key, message):
        # TODO: Implement the above functionality
        for ids in self.hosts_db:
            if ids == message[1].decode():
                self.send(message)
            else:
                msg = "Unknown ID " + message[1].decode()
                statusmes = StatusUpdateMessage.bytes(self.id, message[1].decode(), 0x01, msg)
                self.send(statusmes)
    ...
    def handle_client_quit_message(self, io_device, message):
        # TODO: Implement the above functionality
        for clientUsr in self.hosts_db:
            if io_device == clientUsr:
                io_device.broadcast_message_to_servers(message, io_device.id)
                io_device.broadcast_message_to_adjacent_clients(message, io_device.id)
                self.hosts_db.pop(io_device.id)
            
            for adj in self.adjacent_user_ids:
                self.adjacent_user_ids.pop(io_device.id)
